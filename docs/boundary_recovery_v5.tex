...
\paragraph{Direction.} \SPEC\ The separability classifier's boundary, lifted to ambient space:
\[
  n_a \;=\; \frac{U\nu}{\lVert U\nu \rVert}
...
(\S\ref{sec:stage4}).
\item \textbf{Validate the separability-derived normal.} Not started; needs a $d \ge 2$ harness.
...
\paragraph{Where the error budget probably lives, for step 2.} Responsibilities are
...
Boundary normal from the separability classifier & \SPEC\ --- \textbf{step 2}\\
